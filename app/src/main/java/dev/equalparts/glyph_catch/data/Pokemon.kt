...
object Pokemon {
    ...
    // Generation 2 (Johto) — common families (level/stone evolutions)
    val SENTRET = add(161, "Sentret", Type.NORMAL)
    val FURRET = add(162, "Furret", Type.NORMAL, from = SENTRET at 15)
    val HOOTHOOT = add(163, "Hoothoot", Type.NORMAL, Type.FLYING)
    val NOCTOWL = add(164, "Noctowl", Type.NORMAL, Type.FLYING, from = HOOTHOOT at 20)
    val LEDYBA = add(165, "Ledyba", Type.BUG, Type.FLYING)
    val LEDIAN = add(166, "Ledian", Type.BUG, Type.FLYING, from = LEDYBA at 18)
    val SPINARAK = add(167, "Spinarak", Type.BUG, Type.POISON)
    val ARIADOS = add(168, "Ariados", Type.BUG, Type.POISON, from = SPINARAK at 22)
    val CHINCHOU = add(170, "Chinchou", Type.WATER, Type.ELECTRIC)
    val LANTURN = add(171, "Lanturn", Type.WATER, Type.ELECTRIC, from = CHINCHOU at 27)
    val MAREEP = add(179, "Mareep", Type.ELECTRIC)
    val FLAAFFY = add(180, "Flaaffy", Type.ELECTRIC, from = MAREEP at 15)
    val AMPHAROS = add(181, "Ampharos", Type.ELECTRIC, from = FLAAFFY at 30)
    val HOPPIP = add(187, "Hoppip", Type.GRASS, Type.FLYING)
    val SKIPLOOM = add(188, "Skiploom", Type.GRASS, Type.FLYING, from = HOPPIP at 18)
    val JUMPLUFF = add(189, "Jumpluff", Type.GRASS, Type.FLYING, from = SKIPLOOM at 27)
    val SUNKERN = add(191, "Sunkern", Type.GRASS)
    val SUNFLORA = add(192, "Sunflora", Type.GRASS, from = SUNKERN with Item.SUN_STONE)
    val WOOPER = add(194, "Wooper", Type.WATER, Type.GROUND)
    val QUAGSIRE = add(195, "Quagsire", Type.WATER, Type.GROUND, from = WOOPER at 20)
    val PINECO = add(204, "Pineco", Type.BUG)
    val FORRETRESS = add(205, "Forretress", Type.BUG, Type.STEEL, from = PINECO at 31)
    val SNUBBULL = add(209, "Snubbull", Type.FAIRY)
    val GRANBULL = add(210, "Granbull", Type.FAIRY, from = SNUBBULL at 23)
    val TEDDIURSA = add(216, "Teddiursa", Type.NORMAL)
    val URSARING = add(217, "Ursaring", Type.NORMAL, from = TEDDIURSA at 30)
    val SLUGMA = add(218, "Slugma", Type.FIRE)
    val MAGCARGO = add(219, "Magcargo", Type.FIRE, Type.ROCK, from = SLUGMA at 38)
    val SWINUB = add(220, "Swinub", Type.ICE, Type.GROUND)
    val PILOSWINE = add(221, "Piloswine", Type.ICE, Type.GROUND, from = SWINUB at 33)
    val PHANPY = add(231, "Phanpy", Type.GROUND)
    val DONPHAN = add(232, "Donphan", Type.GROUND, from = PHANPY at 25)
    val LARVITAR = add(246, "Larvitar", Type.ROCK, Type.GROUND)
